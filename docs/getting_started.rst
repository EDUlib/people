...
If you need such overrides, create a file :file:`people/settings/private.py`. This file's values are
read by :file:`people/settings/local.py`, but ignored by Git.
...
Configuring People service to communicate with other IDAs using OAuth requires registering a new client with the authentication
...
:file:`people/settings/local.py`). The table below describes the relevant settings.
